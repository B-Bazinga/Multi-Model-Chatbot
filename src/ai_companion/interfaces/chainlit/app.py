...
async def on_message(message: cl.Message):
    ...
    if output_state.values.get("workflow") == "audio":
        ...
        try:
            ...
            await cl.Message(content=f"[TTS Error] Could not generate audio: {e}").send()
        ...
        try:
            ...
            await cl.Message(content=f"[Image Error] Could not generate image: {e}").send()
    else:
        try:
            await msg.send()
        except Exception as e:
            await cl.Message(content=f"[Error] Could not send message: {e}").send()
...
async def on_audio_end(elements):
    ...
    try:
        ...
        await cl.Message(content=f"[STT Error] Could not transcribe audio: {e}").send()
        return
    ...
    async with AsyncSqliteSaver.from_conn_string(settings.SHORT_TERM_MEMORY_DB_PATH) as short_term_memory:
        ...
        try:
            ...
            await cl.Message(content=f"[Error] Could not process conversation: {e}").send()
            return
